"""Tail a training log into a compact, human-readable progress file.
    ...
    python -m ppo6.watch /tmp/gg.log --out ../runs/ppo5gg/progress.txt
...
def fmt(rows, log_path, t0, phase):
    r = rows[-1]
    f = lambda k, d=0.0: (float(str(r[k]).replace(",", ""))                # noqa: E731
                          if r[k] is not None else d)
    hist = lambda k: [float(str(x[k]).replace(",", ""))                   # noqa: E731
                      for x in rows if x[k] is not None]
    ...
    out.append("")
    out.append(f"  steps {int(f('step')):>13,}   episodes {int(f('ep')):>10,}   updates {int(f('upd')):>6,}"
    ...
    if r["dec"] is not None:
        rng.insert(1, ("decided    ", "dec", 0.0, 100.0, "{:.0f}%"))
    if r["prior"] is not None:
        # The transplant's vital signs. If `prior` decays to 0 or `goal m`
        # collapses, the movement skill has stopped being used.
        rng += [("prior scale", "prior", 0.0, 1.5, "{:+.2f}"),
                ("goal dist  ", "goalm", 0.0, 8.0, "{:.1f}m"),
                ("prior share", "frac",  0.0, 1.0, "{:.2f}")]
    ...
    out.append(f"  {'upd':>7} {'steps':>13} {'wr':>5} {'oacc':>6} "
               f"{'ev':>8} {'H':>7} {'kl':>8}")
    for x in rows[-12:]:
        g = lambda k, d="-": x[k] if x[k] is not None else d              # noqa: E731
        out.append(f"  {g('upd'):>7} {int(str(g('step', 0)).replace(',','')):>13,} {g('wr'):>4}% "
