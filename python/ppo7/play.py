...
def main():
    ...
    ap.add_argument("--eval-seat", type=int, choices=(0, 1), default=None,
                    help="whose win-probability the eval bar shows (0=cyan, "
                    "1=red). Default: the OPPONENT's -- the non-human seat when "
                    "one exists (so vs a human it's always the AI's own view of "
                    "the game, however --swap seated it), else seat 1 (red) "
                    "when spectating with --auto.")
    args = ap.parse_args()
    ...
    eval_recurrent = False   # eval_critic is a MinGRUNet -> carry a hidden state
    eval_h = None            # per-frame hidden for the eval-bar critic (seat 0)
    if args.checkpoint:
        with open(args.checkpoint) as f:
            ckpt = json.load(f)
        # ppo7 checkpoints save AGENT_STATE_DIM (obs + FiGAR hold feature); a
        # bare STATE_DIM would only ever appear on a stripped/legacy export.
        if ckpt.get("stateDim") not in (None, C.STATE_DIM, C.AGENT_STATE_DIM):
            print(f"warning: checkpoint stateDim {ckpt.get('stateDim')} != "
                  f"{C.AGENT_STATE_DIM} — is this a v1 model?")
        ...
        if "agent" in ckpt and isinstance(ckpt["agent"].get("critic"), (list, dict)):
            try:
                _crecs = ckpt["agent"]["critic"]
                eval_recurrent = (isinstance(_crecs, dict)
                                  and _crecs.get("type") == "mingru")
                if eval_recurrent:
                    from .mingru import MinGRUNet
                    eval_critic = MinGRUNet.from_records(_crecs).eval()
                    eval_h = eval_critic.zero_h(1)
                    with torch.no_grad():
                        _out, _ = eval_critic.step(
                            torch.zeros(1, C.AGENT_STATE_DIM), eval_h)
                else:
                    eval_critic = TemporalNet.from_records(_crecs).eval()
                    with torch.no_grad():
                        _out = eval_critic(torch.zeros(1, C.AGENT_STATE_DIM))
                print(f"eval bar: critic loaded ({_out.shape[-1]} outcomes"
                     f"{', recurrent' if eval_recurrent else ''})")
    ...
    # Eval bar seat: the OPPONENT's -- the AI seat when a human is playing
    # (whichever seat --swap put them on), else seat 1 (red), this file's own
    # convention for "the opponent" (see the --opponent flag, always seat 1).
    eval_seat = (args.eval_seat if args.eval_seat is not None
                 else (1 - human_seat) if human_seat is not None else 1)
    eval_seat_name = "cyan" if eval_seat == 0 else "red"
    if eval_critic is not None:
        print(f"eval bar: {eval_seat_name}'s win probability")
    ...
    while running:
        ...
        if res["done"]:
            ...
            if eval_recurrent:
                eval_h = eval_critic.zero_h(1)   # eval bar's own memory too
        ...
        if eval_critic is not None:
            with torch.no_grad():
                # ppo7 critic input = obs + own remaining-hold; the observed
                # seat is treated as free (hold feature 0) for the readout.
                # decision_state(seat) is already framed from THAT seat's own
                # perspective (self/opp blocks swapped, rel dx/dy relative to
                # it) -- eval_seat picks WHOSE win probability this is, not
                # just whose obs happens to get fed in.
                _o = np.append(env.decision_state(eval_seat), 0.0).astype(np.float32)
                _x = torch.from_numpy(_o).float().unsqueeze(0)
                if eval_recurrent:
                    # MUST step through the carried hidden state, not call the
                    # net bare -- MinGRUNet.forward(x, h=None) silently defaults
                    # h to ZERO, which evaluates a memoryless critic every
                    # single frame (the exact bug fixed in trainer.py's
                    # _tail_dist/_value_staller: see ppo7-perf-budget memory).
                    out, eval_h = eval_critic.step(_x, eval_h)
                    out = out[0]
                else:
                    out = eval_critic(_x)[0]
                if out.numel() >= 3:            # categorical: [win, draw, loss(...)]
                    pr = torch.softmax(out, -1)
                    pw, pd = float(pr[0]), float(pr[1])
                    frac = pw + 0.5 * pd        # eval_seat's expected score in [0,1]
            ...
            eval_col = COL_P1 if eval_seat == 0 else COL_P2
            other_col = COL_P2 if eval_seat == 0 else COL_P1
            pygame.draw.rect(screen, other_col, (bx, pad, bw, bh))             # loss top
            fh = int(frac * bh)
            pygame.draw.rect(screen, eval_col, (bx, pad + bh - fh, bw, fh))    # win bottom
            ...
            screen.blit(font.render(f"eval: {eval_seat_name}", True, (200, 240, 230)),
                       (bx - 120, pad + 2))
            screen.blit(font.render(dist, True, (200, 240, 230)), (bx - 120, pad + 19))
