...
def worker_main(wid, offset, n_envs, map_path, raws, shapes, b_obs, b_act,
    ...
    # bonk3 real engine, matching C.ENGINE and the eval harness. It loads
    # decoded maps by name and is the only engine that supports the spawn
    # curriculum (placing disc 1 off its natural spawn).
    def _sf():
...
class VecCollector:
    ...
    def __init__(self, workers, envs_per_worker, map_path,
        ...
        # Read by every worker at every reset; the trainer moves it
        # each cycle to drive the spawn curriculum. lock-free: a
        # torn double read at worst mis-spaces one episode.
