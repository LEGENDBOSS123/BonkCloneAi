...
Observation layout (STATE_DIM = 34):
  ...
  (the draw-clock feature was removed: see config.TIMEOUT_HAZARD)
"""
...
_FOURIER_FREQS = (np.pi * (2.0 ** np.arange(C.FOURIER_L))).astype(np.float32)
...
OUT_LOSS_LATE, OUT_LOSS_EARLY = 3, 4   # time-graded loss classes
...
class LagEnv:
    def __init__(self, sim: EngineSim):
        ...
        # Per-seat frame-stack history; cleared on reset so no motion
        # leaks across an episode boundary.
        self._fhist = (deque(maxlen=C.FRAME_STACK),
                       deque(maxlen=C.FRAME_STACK))
    ...
    def decision_state(self, seat: int) -> np.ndarray:
        """[current, t-1, ... t-(K-1)] concatenated, newest FIRST.

        Newest-first keeps obs[0:34] the present frame, so every existing index
        (own position, own block, the mirror tables) still means what it did
        before stacking.
        """
        cur = self._frame(seat)
        if C.FRAME_STACK <= 1:
            return cur
        h = self._fhist[seat]
        # On reset the history is primed with copies of the first frame, so a
        # fresh episode reports zero motion rather than a spurious jump from
        # whatever the previous episode ended on.
        if not h:
            for _ in range(C.FRAME_STACK - 1):
                h.append(cur.copy())
        out = np.concatenate([cur] + list(h)[: C.FRAME_STACK - 1])
        h.appendleft(cur)
        while len(h) > C.FRAME_STACK - 1:
            h.pop()
        return out
    ...
    def _frame(self, seat: int) -> np.ndarray:
        """One 34-dim observation -- the layout everything indexes into."""
        s = self.sim.players[seat]
        out = np.zeros(C.FRAME_DIM, dtype=np.float32)
        ...
        # Fourier expansion of the position/velocity dims (raw values kept above).
        # block j layout: sin_0,cos_0,sin_1,cos_1,... matching config's mirror map.
        ang = np.outer(out[_FOURIER_BASE], _FOURIER_FREQS)   # (12, L)
        blk = out[C.RAW_FRAME_DIM:].reshape(len(_FOURIER_BASE), C.FOURIER_FEATS)
        blk[:, 0::2] = np.sin(ang)
        blk[:, 1::2] = np.cos(ang)
