...
Cycle (two barriers):
  worker: write obs ─ B1 ─ (main: bookkeeping + inference + write act) ─ B2 ─
          ...
          reset finished episodes; fresh envs idle out the block, matching the
          JS trainer's global decision alignment) ─ repeat.
...
class _WorkerRecorder:
    """Replay writer for the first env of worker 0 (same JSON as ReplayRecorder)."""

    def __init__(self, out_dir, every, tps, action_repeat, input_lag):
        self.dir = out_dir
        self.every = every
        self.meta = {"tps": tps, "actionRepeat": action_repeat, "inputLag": input_lag}
        self.frames = []
        self.seen = 0
    ...
    def frame(self, env, res):
        ...
        if self.seen % self.every == 0:
            ...
            with open(path, "w") as f:
                json.dump({"version": 1, **self.meta, "result": result,
                           "frames": self.frames}, f)
        self.frames = []


def worker_main(wid, offset, n_envs, map_path, k, raws, shapes, b_obs, b_act,
    ...
    recorder = None
    if wid == 0 and replay_cfg:
        recorder = _WorkerRecorder(**replay_cfg)
...
class VecCollector:
    ...
    def __init__(self, workers, envs_per_worker, state_dim, map_path, k,
                 replay_dir=None, replay_every=500, tps=30, input_lag=2):
        ...
        shapes = {
            "obs": (self.E, 2, state_dim),
            "act": (self.E, 2),
            "brew": (self.E, 2),
            "done": (self.E, 4),
        }
        raws = {
            "obs": ctx.RawArray(ctypes.c_float, self.E * 2 * state_dim),
        ...
        for w in range(workers):
            ...
            if w == 0 and replay_dir:
                replay_cfg = {"out_dir": str(replay_dir), "every": replay_every,
                              "tps": tps, "action_repeat": k, "input_lag": input_lag}
            p = ctx.Process(
                target=worker_main,
                args=(w, w * envs_per_worker, envs_per_worker, str(map_path), k,
    ...
    def sync_obs(self):
        """Wait for all workers' observations; returns COPIES of the shared
        block results (obs stays a view — copy rows you keep)."""
        self.b_obs.wait()
