...
def worker_main(wid, offset, n_envs, map_path, raws, shapes, b_obs, b_act,
                stop_flag, seed, replay_cfg, spawn_frac):
    ...
    for env in envs:
        env.reset(spawn_frac=spawn_frac.value)
        ...
        for _ in range(C.ACTION_REPEAT):
            for i, env in enumerate(envs):
                ...
                if res["done"]:
                    ...
                    # read the live curriculum value: the main process
                    # advances it as episodes accumulate
                    env.reset(spawn_frac=spawn_frac.value)
...
class VecCollector:
    ...
    def __init__(self, workers, envs_per_worker, map_path,
        ...
        # Curriculum fraction, written by the main process each cycle and read
        # by workers when they reset an env. lock=False: a torn read of a
        # double would at worst pick a neighbouring schedule value.
        self.spawn_frac = ctx.Value(ctypes.c_double,
                                    0.0 if C.SPAWN_CURRICULUM else 1.0,
                                    lock=False)
