"""Head-to-head evaluation between two checkpoints — the FIXED-reference test
that ELO can't give you.

ELO is measured against a rolling buffer of the main's recent selves, so it
plateaus whether or not the agent is improving (the yardstick rises with it).
To actually know if the current agent is stronger than an older one, play them
directly against a frozen opponent:

    python -m bonk.eval_h2h --a runs/ppo-mp/bonk-ppo-ep6500000-*.json \
                            --b runs/ppo-mp/bonk-ppo-ep5500000-*.json --games 2000

If A beats B ~90%, the agent improved over that span (flat ELO was a treadmill
artifact). If it's ~50%, it's genuinely stuck. Sides are swapped each half so
spawn asymmetry cancels; lag is randomized per episode exactly as in training.
"""
...
def load_actor(path_glob: str) -> MLP:
...
def main():
    ...
    print(f"{args.games} games/side, {'greedy' if args.greedy else 'sampled'}, "
          f"lag {'randomized' if getattr(C, 'INPUT_LAG_RANDOM', False) else C.INPUT_LAG}")

    half = args.games
    # Side 1: A on seat 0. Side 2: A on seat 1 (swap to cancel spawn bias).
    a_w1, b_w1, d1 = play(net_a, net_b, map_json, half, args.envs, args.greedy)
    b_w2, a_w2, d2 = play(net_b, net_a, map_json, half, args.envs, args.greedy)
