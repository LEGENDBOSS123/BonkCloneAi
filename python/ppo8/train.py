...
from .ppo import (PPOAgent, entropy_coef_at, duration_entropy_coef_at,
                  intrinsic_coef_at)
...
class Trainer:
    ...
    def __init__(self, collector: VecCollector, agent: PPOAgent):
        ...
        # Per-row flag: the MAIN's opponent on this row is a STALLER exploiter.
        # Used to score the main's timeout as 0 (neutral) instead of -1, and to
        # re-weight the value baseline accordingly (see run_update).
        self.b_staller = np.zeros((self.T, E), dtype=bool)
        # FiGAR: the hold-duration index chosen at each row, and whether the row
        # was a FREE decision. The actor trains only on FREE rows (held rows were
        # forced repeats); the critic + GAE use every row.
        self.b_dur = np.zeros((self.T, E), dtype=np.int64)
        self.b_free = np.zeros((self.T, E), dtype=bool)
        self.b_odur = np.zeros((self.T, E), dtype=np.int64)
        self.b_ofree = np.zeros((self.T, E), dtype=bool)
        self.t = 0
        ...
        # Per-seat FiGAR hold state: cycles left on the current hold + the action
        # being held. A seat re-decides only when its hold reaches 0.
        self.hold0 = np.zeros(E, dtype=np.int64)
        self.held_a0 = np.zeros(E, dtype=np.int64)
        self.hold1 = np.zeros(E, dtype=np.int64)
        self.held_a1 = np.zeros(E, dtype=np.int64)
        ...
        self.staller_mask = np.zeros(E, dtype=bool) # opp[i] is a STALLER exploiter
    ...
    def cycle(self):
        ...
        if self.p_valid:
            ...
            self.b_v[t] = self.p_v
            self.b_r[t] = self.p_r
            # Exploiter phase: seat 0 IS the exploiter. A STALLER exploiter
            # scores a DRAW as a WIN (EXPLOITER_STALLER_DRAW_REWARD) -- it is
            # trained to time the frozen main out; a normal exploiter uses
            # EXPLOITER_DRAW_REWARD. Only the LEARNER's reward is touched.
            if self.league.phase == "exploiter" and ended.any():
                dr = (C.EXPLOITER_STALLER_DRAW_REWARD
                      if self.league.exp_is_staller else C.EXPLOITER_DRAW_REWARD)
                if dr != C.DRAW_REWARD:
                    drew = ended & ((done[:, 3] > 0)
                                    | ((done[:, 1] > 0) & (done[:, 2] > 0)))
                    if drew.any():
                        self.b_r[t, drew] = dr
            self.b_d[t] = ended
            # Record which rows faced a STALLER opponent (main phase). The main's
            # timeout vs a staller is scored 0 (neutral), not the usual -1 -- the
            # value baseline is re-weighted to match in run_update.
            self.b_staller[t] = self.staller_mask
            if self.league.phase != "exploiter" and ended.any():
                st_draw = self.staller_mask & ended & ((done[:, 3] > 0)
                              | ((done[:, 1] > 0) & (done[:, 2] > 0)))
                if st_draw.any():
                    self.b_r[t, st_draw] = 0.0
        ...
        # A new episode starts free (no carried hold from the last one).
        if ended.any():
            self.hold0[ended] = 0
            self.hold1[ended] = 0
        ...
        # Append each seat's OWN remaining hold, normalised, as the last obs
        # feature (0 where free). This is what the net sees; it makes a mid-hold
        # state Markov for the critic.
        f0 = (self.hold0.astype(np.float32) / self._maxdur)[:, None]
        l_states = np.concatenate([np.ascontiguousarray(obs[:, 0]), f0], axis=1)
        if len(cur):
            f1 = (self.hold1[cur].astype(np.float32) / self._maxdur)[:, None]
            c_states = np.concatenate(
                [np.ascontiguousarray(obs[cur, 1]), f1], axis=1)
            both = np.concatenate([l_states, c_states])
        else:
            c_states = None
            both = l_states
        # Pad batch height to a multiple of 512 (MPS caches a kernel graph per
        # shape and never evicts -- see ppo6 note).
        n_real = both.shape[0]
        pad = -n_real % 512
        if pad:
            both = np.concatenate(
        ...
        sa, sd, slp, sv = acts[:self.E], durs[:self.E], lps[:self.E], vals[:self.E]
        free0 = self.hold0 == 0
        self.held_a0[free0] = sa[free0]
        self.hold0[free0] = self._durs[sd[free0]]
        actions[:, 0] = self.held_a0
        self.p_s[:] = l_states
        self.p_a[:] = self.held_a0
        self.p_dur[:] = sd                 # duration index (used only where free)
        self.p_free[:] = free0
        self.p_lp[:] = slp                 # joint logp (used only where free)
        self.p_v[:] = sv                   # value at EVERY row (GAE needs it)
        self.p_r[:] = 0.0
        self.hold0 -= 1                     # decrement after recording the row

        # ── seat 1 ("current" = self-play): same, on the cur subset ──────────
        self.po_m[:] = self.cur_mask
        self.po_r[:] = 0.0
        self.po_free[:] = False
        if len(cur):
            ca, cd = acts[self.E:], durs[self.E:]
            clp, cv = lps[self.E:], vals[self.E:]
            freec = self.hold1[cur] == 0
            self.held_a1[cur[freec]] = ca[freec]
            self.hold1[cur[freec]] = self._durs[cd[freec]]
            actions[cur, 1] = self.held_a1[cur]
            self.po_s[cur] = c_states
            self.po_a[cur] = self.held_a1[cur]
            self.po_dur[cur] = cd
            self.po_free[cur] = freec
            self.po_lp[cur] = clp
            self.po_v[cur] = cv
            self.hold1[cur] -= 1
        self.p_valid = True
        ...
        for (kind, idx), idxs in groups.items():
            if kind == "idle":
                ...
                continue
            # Frozen pool / exploiter / frozen-main opponents ALSO hold their
            # action for a sampled duration (they were trained that way). Track
            # seat-1 hold state for them exactly like the learner; only re-query
            # the frozen net where the hold has expired (free -> hold feature 0).
            idxs = np.asarray(idxs)
            net = self.league.opponent_net(kind, idx)
            free1 = self.hold1[idxs] == 0
            if free1.any():
                fidx = idxs[free1]
                o = np.ascontiguousarray(obs[fidx, 1])
                o = np.concatenate([o, np.zeros((len(fidx), 1), np.float32)], 1)
                a_op, d_op = self.agent.act_actions(net, o)
                self.held_a1[fidx] = a_op
                self.hold1[fidx] = self._durs[d_op]
            actions[idxs, 1] = self.held_a1[idxs]
            self.hold1[idxs] -= 1
    ...
    @staticmethod
    def _gae_intrinsic(r, v, d, last_v):
        """GAE for the INTRINSIC reward stream — its own (shorter) discount
        INTRINSIC_GAMMA, kept separate from the extrinsic GAMMA=1 (ppo8)."""
        T = r.shape[0]
        adv = np.zeros_like(r); gae = np.zeros(r.shape[1], dtype=np.float32)
        g = C.INTRINSIC_GAMMA
        for t in range(T - 1, -1, -1):
            nt = 1.0 - d[t]
            next_v = last_v if t == T - 1 else v[t + 1]
            delta = r[t] + g * next_v * nt - v[t]
            gae = delta + g * C.GAE_LAMBDA * nt * gae
            adv[t] = gae
        return adv, adv + v
    ...
    def _value_staller(self, states):
        """Critic value with the DRAW atom re-weighted to 0 (MAIN_VS_STALLER_ATOMS)
        -- the value baseline that matches a 0-reward timeout vs a staller."""
        agent = self._active_agent()
        satoms = torch.tensor(C.MAIN_VS_STALLER_ATOMS, dtype=torch.float32,
                              device=agent.device)
        with torch.no_grad():
            out = agent.critic(torch.from_numpy(
                np.ascontiguousarray(states, dtype=np.float32)).to(agent.device))
    ...
    def run_update(self):
        ...
        # Vs a STALLER, re-weight the value baseline so the DRAW atom is 0 (the
        # main's timeout return is 0 there). Without this, a 0 return against a
        # V that expects -1 would read as a +1 advantage -- rewarding the draw,
        # the exact opposite of "learn to break the turtle".
        v_col = self.b_v[:T].astype(np.float32).copy()
        sm = self.b_staller[:T]
        if C.CRITIC_CATEGORICAL and sm.any():
            fidx = np.nonzero(sm.reshape(-1))[0]
            v_col.reshape(-1)[fidx] = self._value_staller(
                self.b_s[:T].reshape(T * E, D)[fidx])
            cut_st = (self.b_d[T - 1] <= 0) & self.b_staller[T - 1]
            if cut_st.any():
                last_v = np.where(cut_st, self._value_staller(self.p_s), last_v)
        adv, ret = self._gae_columns(r_used, v_col, self.b_d[:T], last_v)
        # ── ppo8: fold the ensemble-disagreement intrinsic advantage in ────────
        # r_int = ensemble value-std on the learner states; its own GAE (shorter
        # horizon, own value head) gives A_int; the actor sees A_ext + beta*A_int.
        # The ensemble + intrinsic head are then fit on this same aligned data.
        # Intrinsic reward on the EXPLOITER (the innovation engine): exploiters
        # best-respond to a frozen target and need to EXPLORE to find a hole, so
        # the disagreement bonus belongs on them, not the main (main-side helped
        # nothing -- the main's job is to absorb/counter, not invent). Active only
        # in the exploiter sub-phase; uses the exploiter's own ensemble.
        _ea = self._active_agent()
        if (getattr(_ea, "use_ensemble", False) and C.LEAGUE_ENABLED
                and self.league.phase == "exploiter"):
            flat_s = self.b_s[:T].reshape(T * E, D)
            r_int_raw = _ea.ensemble_disagreement(flat_s)                 # [T*E] raw std
            r_int = _ea.normalize_intrinsic(r_int_raw).reshape(T, E)      # ~unit scale
            v_int = _ea.intrinsic_value(flat_s).reshape(T, E)
            last_vint = np.where(self.b_d[T - 1] > 0, 0.0,
                                 _ea.intrinsic_value(self.p_s)).astype(np.float32)
            adv_int, ret_int = self._gae_intrinsic(r_int, v_int, self.b_d[:T], last_vint)
            beta = C.EXPLOITER_INTRINSIC_COEF     # fixed: exploiters explore hard throughout
            adv = adv + beta * adv_int
            self._last_intrinsic = (_ea, flat_s.copy(), ret.reshape(-1).copy(),
                                    ret_int.reshape(-1).copy(), float(r_int.mean()), beta)
        ...
        durations = self.b_dur[:T].reshape(-1).copy()   # FiGAR duration index
        free = self.b_free[:T].reshape(-1).copy()        # actor trains free rows
        ...
        if m.any():
            ...
            durations = np.concatenate([durations, self.b_odur[:T].reshape(-1)[sel]])
            free = np.concatenate([free, self.b_ofree[:T].reshape(-1)[sel]])
        ...
        if C.MIRROR_MODE == "duplicate":
            ...
            # duration index and the free mask are mirror-invariant.
        ...
        # The duration head keeps its own (stronger) entropy in BOTH phases --
        # the FiGAR collapse is opponent-independent, so exploiters need it too.
        dec = duration_entropy_coef_at(self.league.main_steps)
        ...
        stats = self._active_agent().update(rollout, entropy_coef=ec,
                                            shaping_w=sw,
                                            duration_entropy_coef=dec)
        # ppo8: fit the EXPLOITER's ensemble + intrinsic head on its aligned data
        li = getattr(self, "_last_intrinsic", None)
        if li is not None:
            _ea, fs, er, ir, rint_mean, beta = li
            istats = _ea.train_intrinsic(fs, er, ir)
            self._last_intrinsic = None
            if istats:
                stats.update({"r_int": rint_mean, "beta": beta, **istats})
...
def main():
    ...
    ap.add_argument("--config",
                    help="JSON file of config overrides (per-phase tuning, e.g. "
                         "CLIP_EPS/ACTOR_LR/EPOCHS). Applied before phase flags.")
    ...
    if args.config:
        ...
        for _k, _v in _ov.items():
            if _k.startswith("_"):
                continue                          # _comment etc. are ignored
            if not hasattr(C, _k):
                raise SystemExit(f"--config {args.config}: unknown config key {_k!r}")
            setattr(C, _k, _v)
            _applied[_k] = _v
        print(f"  config overrides from {args.config}: {_applied}")
