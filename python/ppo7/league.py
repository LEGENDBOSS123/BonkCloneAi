...
# ── frozen-net construction ─────────────────────────────────────────────────
# Pool members must be the SAME class as the live actor, or the state_dict keys
# mismatch on freeze and the records mismatch on load. Getting this wrong is how
# a phase-3 run once crashed: a snapshot tried to load minGRU weights into a
# conv net, thousands of episodes in.
def _make_net(state_dim, out_dim):
    """An empty frozen-pool net matching the live actor's architecture."""
    if getattr(C, "RECURRENT", False):
        return MinGRUNet(state_dim, C.HIDDEN, out_dim, gru_hidden=C.MINGRU_HIDDEN)
    return TemporalNet(state_dim, C.HIDDEN, out_dim)


def _net_from_records(recs):
    """Rebuild a frozen net from a checkpoint's records, dispatching on the
    `type` tag the records carry."""
    if isinstance(recs, dict) and recs.get("type") == "mingru":
        return MinGRUNet.from_records(recs)
    return TemporalNet.from_records(recs)
...
class League:
    ...
    def is_staller(self, kind, idx) -> bool:
        """True if opponent (kind, idx) is a graduated STALLER exploiter -- the
        main scores a timeout against it as 0, not -1."""
        return (kind == "exp" and idx is not None
                and 0 <= idx < len(self.exploiters)
                and self.exploiters[idx].get("staller", False))
    ...
    def start_exploiter(self, force_staller=None):
        """`force_staller`: None = the normal random draw; True/False pins the
        type -- for deliberately testing one type (e.g. `--exploiter-type` at
        launch), not used by the normal trigger path."""
        ...
        # Decide the exploiter TYPE first: a STALLER is rewarded for drawing the
        # frozen main (draw atom = +1), so its critic needs staller atoms.
        self.exp_is_staller = (bool(force_staller) if force_staller is not None
                               else (np.random.random() < C.EXPLOITER_STALLER_PROB))
        atoms = (C.EXPLOITER_STALLER_ATOMS if self.exp_is_staller
                 else C.EXPLOITER_ATOMS)
        self.exp_agent = PPOAgent(self.agent.state_dim, self.agent.num_actions,
                                  device=str(self.agent.device), atoms=atoms,
                                  is_exploiter=True, is_staller=self.exp_is_staller)
        # Per-exploiter starting entropy: log-uniform draw (population search over
        # explore/exploit). Each exploiter anneals from its own start to EC_END.
        if getattr(C, "EXPLOITER_EC_RANDOM", False):
            lo, hi = C.EXPLOITER_EC_START_MIN, C.EXPLOITER_EC_START_MAX
            self.exp_ec_start = float(np.exp(np.random.uniform(np.log(lo), np.log(hi))))
        else:
            self.exp_ec_start = C.EXPLOITER_EC_START
        ...
        self.exp_agent.resample_param_noise()   # initial perturbation from the seed
    ...
    def exploiter_entropy_coef(self):
        ...
        ec0 = getattr(self, "exp_ec_start", C.EXPLOITER_EC_START)  # per-exploiter draw
        frac = min(1.0, self.phase_steps / C.EXPLOITER_EC_DECAY_STEPS)
        return ec0 + (C.EXPLOITER_EC_END - ec0) * frac
    ...
    def _freeze(self, src_net, device="cpu"):
        ...
        device."""
        # Match the actor's real class (MinGRUNet when recurrent, else TemporalNet)
        # and output width, else the state_dict keys mismatch on freeze.
        net = _make_net(self.agent.state_dim, src_net.out.out_features)
    ...
    def load(self, obj, episode_count):
        def frozen(records):
            net = _net_from_records(records)          # pool nets live on CPU (see _freeze)
            for p in net.parameters():
                p.requires_grad_(False)
            return net
