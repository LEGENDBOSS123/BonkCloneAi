"""Pre-train the movement CORE by RL on a dense goal-reaching reward.
    ...
    python -m ppo6.pretrain_move --iters 300 --out ../runs/move_core.json
...
def main():
    ...
    ap.add_argument("--ent-end", type=float, default=0.0005,
                    help="entropy coef is annealed --ent -> --ent-end")
    ...
    # This objective is high variance across seeds: identical settings have
    # produced both +2.96 m and -2.22 m mean closure. Two fixes -- anneal the
    # entropy bonus so the policy actually commits, and KEEP THE BEST model
    # rather than whatever the last iteration happened to be. Without the
    # second, a good run can still be saved at a bad moment.
    best_score, best_state, ema = -1e9, None, None
    for it in range(1, args.iters + 1):
        frac = (it - 1) / max(1, args.iters - 1)
        ent_coef = args.ent + (args.ent_end - args.ent) * frac
        for e in envs:
            e.reset(spawn_frac=1.0)
        ...
        for _ in range(args.epochs):
            ...
            for s in range(0, n, 4096):
                ...
                loss = -torch.min(s1, s2).mean() - ent_coef * ent \
        ...
        # Closure discounted by survival: a policy that "closes" by diving off
        # the map is not a movement skill.
        closed = float((a_d0 - a_df).mean()) * float(alive.mean())
        ema = closed if ema is None else 0.9 * ema + 0.1 * closed
        if it > args.iters // 10 and ema > best_score:
            best_score = ema
            best_state = {k: v.detach().clone() for k, v in net.state_dict().items()}
        if it % 10 == 0 or it == 1:
            print(f"  {it:>5} {a_df.mean():>10.2f}m {a_d0.mean():>10.2f}m "
                  f"{(a_d0 - a_df).mean():>7.2f}m {ent_v:>8.3f} "
                  f"{alive.mean()*100:>5.0f}% {ema:>7.2f} "
                  f"{time.perf_counter() - t0:>5.0f}")

    if best_state is not None:
        net.load_state_dict(best_state)
        print(f"\n  restored BEST checkpoint (ema closure {best_score:+.2f} m)")
    Path(args.out).parent.mkdir(parents=True, exist_ok=True)
    with open(args.out, "w") as f:
        # v2 saves the WHOLE skill: l_in and l_out as well as the core.
        # v1 kept only the middle, which meant the transplanted module's input
        # and output spaces were arbitrary 64-d codes the actor had to learn
        # from scratch -- as hard as learning movement itself, so the
        # transplant bought nothing. Keeping l_in/l_out makes the interface
        # GROUNDED: inputs are (own_state, goal) and outputs are action logits.
        sd = lambda m: {k: v.tolist() for k, v in m.state_dict().items()}
        json.dump({"format": "move-core-v2", "d_emb": C.MOVE_EMB,
                   "hidden": C.MOVE_HIDDEN,
                   "own_dim": OWN_DIM, "goal_dim": GOAL_DIM,
                   "num_actions": C.NUM_ACTIONS,
                   # Goal sampling ranges, in the SCALED units the net saw.
                   # The actor bounds its emitted goal to these so the frozen
                   # stack is never asked about a state it has not seen.
                   "goal_radius": C.MOVE_GOAL_RADIUS * C.POS_SCALE,
                   "goal_vx": 4.0 * C.VEL_SCALE,
                   "goal_vy": 2.0 * C.VEL_SCALE,
                   "emb_mean": net.emb_mean.tolist(),
                   "emb_std": net.emb_var.sqrt().tolist(),
                   "l_in": sd(net.l_in), "l_out": sd(net.l_out),
                   "core": sd(net.core)}, f)
    print(f"\n  CORE saved -> {args.out}")
